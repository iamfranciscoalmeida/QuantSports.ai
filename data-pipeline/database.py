...
from config import VITE_SUPABASE_URL, VITE_SUPABASE_ANON_KEY, SUPABASE_SERVICE_ROLE_KEY
...
class MatchDatabase:
    def __init__(self, use_service_role: bool = True):
        ...
        Args:
            use_service_role: Use service role key for elevated permissions (default: True for ETL)
        """
        if use_service_role and SUPABASE_SERVICE_ROLE_KEY:
            logger.info("Using Supabase service role key for database operations")
            self.supabase: Client = create_client(VITE_SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY)
        else:
            logger.info("Using Supabase anonymous key for database operations")
            self.supabase: Client = create_client(VITE_SUPABASE_URL, VITE_SUPABASE_ANON_KEY)
